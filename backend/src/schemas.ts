...
export const itemBodySchema = z.object({
  ...
  notes:       z.preprocess((v) => v === '' ? null : v, z.string().max(500).optional().nullable()),
  expiry_date: z.preprocess((v) => v === '' ? null : v, z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data inválida (YYYY-MM-DD)').optional().nullable()),
  image_url:   z.preprocess((v) => v === '' ? null : v, z.string().url().optional().nullable()),
});
