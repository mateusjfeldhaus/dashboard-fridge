...
async function deleteStorageImage(imageUrl: string | null | undefined): Promise<void> {
  if (!imageUrl) return;
  try {
...
router.post('/', upload.single('image'), validate(itemBodySchema), async (req, res) => {
  try {
    const { name, category, quantity, unit, notes, expiry_date } = req.body as {
      name: string; category: string; quantity: number; unit: string;
      notes?: string | null; expiry_date?: string | null;
    };
...
router.put('/:id', upload.single('image'), validate(idParamSchema, 'params'), validate(itemBodySchema), async (req, res) => {
  try {
    const { name, category, quantity, unit, notes, expiry_date } = req.body as {
      name: string; category: string; quantity: number; unit: string;
      notes?: string | null; expiry_date?: string | null; image_url?: string | null;
    };
    ...
    if (!existing.length) { res.status(404).json({ error: 'Item not found' }); return; }
    const oldImageUrl: string | null = existing[0].image_url;
